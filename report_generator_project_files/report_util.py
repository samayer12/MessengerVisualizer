...
class ReportContext:
    ...
    def assign_part_numbers(self, report: Report):
        number_counts = {ReportPartType.Paragraph: 0, ReportPartType.Figure: 0, ReportPartType.Table: 0,
                         ReportPartType.Section: 0, ReportPartType.All: 0}
...
class HTMLReportContext(ReportContext):
    ...
    @staticmethod
    def _write_cross_reference(context: "HTMLReportContext", html_file: TextOutputStream, text_part: TextPart,
                               level: int = 2):
        cross_reference = typing.cast(CrossReference, text_part)
        html_file.write("<a href='#{}' class='cross_reference'>{}</a>".format(cross_reference.report_part.part_number,
                                                                              cross_reference))

    @staticmethod
    def _write_sub_part_section_title(context: "HTMLReportContext", html_file: TextOutputStream, section: Section,
                                      level: int = 2):
        html_file.write(
            "<h{}>{} <a href='#top' class='jump'>&#8679;</a></h{}>".format(level, html.escape(section.title), level))

    @staticmethod
    def _decorate_anchor_name(write_strategy):
        def anchor_strategy(context: "HTMLReportContext", html_file: TextOutputStream, report_part: ReportPart,
                            level: int = 2):
            html_file.write(
                "<a name='{}' class='part_anchor'>{}</a>".format(report_part.part_number, report_part.part_number))
            write_strategy(context, html_file, report_part, level)

        return anchor_strategy

    @staticmethod
    def _write_paragraph(context: "HTMLReportContext", html_file: TextOutputStream, report_part: ReportPart,
                         level: int = 2):
        html_file.write("<p>")
    ...
    @staticmethod
    def _write_table(context: "HTMLReportContext", html_file: TextOutputStream, report_part: ReportPart,
                     level: int = 2):
    ...
    @staticmethod
    def _write_figure(context: "HTMLReportContext", html_file: TextOutputStream, report_part: ReportPart,
                      level: int = 2):
        ...
        html_file.write(
            "<figcaption>Figure {}. {}</figcaption>".format(figure.part_type_number, html.escape(figure.caption)))
        html_file.write("</figure>")
        context.figure_count += 1

    @staticmethod
    def _write_section(context: "HTMLReportContext", html_file: TextOutputStream, report_part: ReportPart,
                       level: int = 2):
        ...
        for child_report_part in section.children:
            context.part_write_strategies[child_report_part.get_type()](context, html_file, child_report_part,
                                                                        level + 1)
